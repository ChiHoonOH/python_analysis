...
def ex3_face():
    ...
    face_data = face_all.data[face_all.target == 10]
    W1 = pca1.fit_transform(face_data)
    # faces2 = pca1.inverse_transform(W1)
    # show_image(faces2)

    # 각 얼굴로부터 얻은 평준화된 데이터
    face_mean = pca1.mean_.reshape(64, 64)
    face_p1 = pca1.components_[0].reshape(64, 64)
    face_p2 = pca1.components_[1].reshape(64, 64)
    '''
    plt.subplot(131)
